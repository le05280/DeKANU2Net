...
def main(args):
    ...
    train_dataset = DUTSDataset(args.data_path, train=True, transforms=SODPresetTrain([640, 640], crop_size=480))
    val_dataset = DUTSDataset(args.data_path, train=False, transforms=SODPresetEval([640, 640]))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_interval == 0 or epoch == args.epochs - 1:
            # eval_interval validation every epoch to reduce validation frequency to save training time
            ...
            if current_mae >= mae_info and current_f1 <= f1_info:
                # Refresh the current_mae, current_f1 values to make sure we save the best ones into model_best.pth
                current_mae = mae_info
                current_f1 = f1_info
...
def parse_args():
    ...
    parser.add_argument("--data-path", default="Datas/", help="DUTS root")
