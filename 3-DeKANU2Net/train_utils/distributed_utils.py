...
class F1Score(object):
    ...
    def update(self, pred: torch.Tensor, gt: torch.Tensor):
        ...
        self.pre_mean = self.precision_cum / self.num_cum
        self.rec_mean = self.recall_cum / self.num_cum
        self.f1_mean = (1 + 0.3) * self.pre_mean * self.rec_mean / (0.3 * self.pre_mean + self.rec_mean + 1e-8)

    def compute(self):
        max_f1 = torch.amax(self.f1_mean).item()
        return {
            'pre_mean': self.pre_mean,
            'rec_mean': self.rec_mean,
    ...
    def __str__(self):
        ...
        return f'maxF1: {metrics["max_f1"]:.3f}, pre_mean: {metrics["pre_mean"]}, rec_mean: {metrics["rec_mean"]}, f1_mean: {metrics["f1_mean"]}'
'''
