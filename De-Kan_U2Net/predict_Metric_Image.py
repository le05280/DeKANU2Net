...
def main():
    ...
    # 拼接所有模型的结果
    final_comparison_img = None
    for model_result_folder in all_models_results:
        for img_name in os.listdir(model_result_folder):
            if img_name.endswith('_comparison.png'):
                img_path = os.path.join(model_result_folder, img_name)
                img = cv2.imread(img_path, cv2.IMREAD_COLOR)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                if final_comparison_img is None:
                    final_comparison_img = img
                else:
                    final_comparison_img = np.hstack((final_comparison_img, img))
